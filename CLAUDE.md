...
Landing page for the [Model Graph Tools](https://github.com/model-graph-tools) project, served at https://model-graph-tools.github.io. It's a static site — a single `index.html` with inline CSS/JS, plus an `mgt/install.sh` script for the `mgt` CLI.
...
- **`mgt/install.sh`** — shell installer that downloads the latest `mgt` binary from GitHub Releases. Supports Linux (x86_64) and macOS (x86_64, aarch64).
